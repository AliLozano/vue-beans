...
describe('loadLazyFields', () => {
  test('Test that lazy props are replaced', () => {
    const getterFn = vitest.fn(() => 10)
    const setterFn = vitest.fn((value: number) => {})
    const getterBuilder = vitest.fn((name: string, instance: Foo) => getterFn)
    const setterBuilder = vitest.fn((name: string, instance: Foo) => setterFn)
