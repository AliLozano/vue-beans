...
export declare type GetterFunction<T, B = GenericBean<unknown>> = (
  name: string,
  instance: B
) => () => T

export declare type SetterFunction<T, B = GenericBean<unknown>> = (
  name: string,
  instance: B
) => (newValue: T) => void
...
export function loadLazyFields<T extends GenericBean<T>>(instance: T): T {
  Object.entries(instance)
    ...
    .forEach(([key, value]) => {
      ...
      const setter = lazyInjector[LAZY_SETTER]
        ? lazyInjector[LAZY_SETTER]!!(key, instance)
        : undefined
