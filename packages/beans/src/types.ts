...
export type EventBus<O> = O extends object ? {
    [K in RequiredKeys<O>]: {value: InferPropType<O[K]>, type: string} & ((value: InferPropType<O[K]>) => void);
} & {
    [K in OptionalKeys<O>]: {value?: InferPropType<O[K]>, type: string} & ((value?: InferPropType<O[K]>) => void);
} : {
    [K in string]: any;
};
...
export type GenericBean<T> = Record<keyof T, unknown> & {
...
export declare type BeanInitializer<T extends GenericBean<T>> = (instance: T) => void;

export declare type BeanConstructor<T extends GenericBean<T>> = (new () => T) & {
    [IS_REACTIVE]?: boolean;
...
export declare type Decorator<T> = (instance: T | (new () => T), callback: string) => void;