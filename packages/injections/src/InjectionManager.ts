...
export class InjectionManager {
  private static readonly globalFactories: Record<string, BeanConstructorOrBuilder<GenericBean<any>>> = {};

  private readonly localFactories: Record<string, BeanConstructorOrBuilder<GenericBean<any>>> = {};
  ...
  static registerGlobalBeanFactory(name: string, initializer: BeanConstructorOrBuilder<any>, _override = false): void {
    if (process.env.NODE_ENV === 'development') console.info(`Bean Factory ${name} registred globalmente`);
    this.globalFactories[name] = this.globalFactories[name] || initializer;
  }

  static unRegisterGlobalBeanFactory(name: string): void {
    if (process.env.NODE_ENV === 'development') console.info(`Delete global factory ${name}`);
    delete this.globalFactories[name];
  }

  registerBeanFactory(name: string, initializer: BeanConstructorOrBuilder<any>): void {
    if (process.env.NODE_ENV === 'development') console.info(`Bean Factory ${name} registred`);
    this.localFactories[name] = initializer;
  }

  useService<T extends GenericBean<T>>(name: string, defaultValue?: BeanConstructorOrBuilder<T>): Ref<T> {
    ...
    const beanFactory = this.localFactories[name] || InjectionManager.globalFactories[name] || defaultValue;
    ...
    try {
      if (process.env.NODE_ENV === 'development') console.time(`Bean for ${name} created`);
      ...
      if (process.env.NODE_ENV === 'development') console.timeEnd(`Bean for ${name} created`);
      return obj;
    } catch (e) {
  ...
  protected createFactory<T extends GenericBean<T>>(builder: BeanConstructorOrBuilder<T>) {
    return (): T => {
  ...
  useAnonymousService<T extends GenericBean<T>>(builder: BeanConstructorOrBuilder<T>): Ref<T> {
...
export function setCurrentInjectionManager(instance: InjectionManager | null): void {
