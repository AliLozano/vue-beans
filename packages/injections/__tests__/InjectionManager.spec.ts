...
describe('InjectionManager', () => {
  ...
  test('test useService call setup method', () => {
    const setup = vitest.fn(() => undefined)
  ...
  test('test sharing key between two services', () => {
    ...
    ;(Foo as BeanConstructorInjectable<Foo>)[INJECTOR_IDENTIFIER] =
      'FooIdentifier'
  ...
  test('test cyclic reference injection', () => {
    ...
    expect(
      fooInstance.value === fooInstance.value.bar.value.foo.value
    ).toBeTruthy()
  })
})
