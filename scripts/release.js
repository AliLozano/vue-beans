...
const isCorePackage = pkgName => {
  ...
  if (pkgName === 'vue-beans') {
    return true
  }

  return (
    pkgName.startsWith('@vue-beans') &&
    packages.includes(pkgName.replace(/^@vue-beans\//, ''))
  )
}

const renamePackageToCanary = pkgName => {
  if (pkgName === 'vue-beans') {
    return '@vue-beans/canary'
  }
...
async function main() {
  ...
  if (isCanary) {
    ...
    try {
      const pkgName = renamePackageToCanary('vue-beans')
